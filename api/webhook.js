...
const FALLBACK_MAP = {
    ...
    'dien bien': 22,
    'an giang': 8,
    'đà nẵng': 5, // Giả sử ID cho Đà Nẵng, thay bằng thực tế
    'da nang': 5,
    'hà nội': 1, // Giả sử ID cho Hà Nội, thay bằng thực tế
    'ha noi': 1,
    // Thêm khác nếu cần
};
...
function formatDepartureDate(thoiGian) {
    ...
    try {
        ...
        if (Array.isArray(thoiGian)) {
            // Parse array: Tìm timestamp hợp lý (bỏ range object, lấy ISO string gần nhất với "ngày mai")
            dateStr = thoiGian.find(item => typeof item === 'string' && item.includes('T')) || thoiGian[thoiGian.length - 2]; // Ví dụ: lấy '2025-11-08T00:00:00+07:00' cho ngày mai
        ...
        if (dateStr) {
            ...
            if (isNaN(date)) throw new Error('Invalid date');
            console.log(`Formatted departure: ${date.toISOString()}`);
            return date.toISOString(); // Giữ full ISO, backend sẽ handle
        }
        return null;
    } catch (error) {
        console.error("Lỗi format ngày:", error);
...
module.exports = async function handler(req, res) {
    ...
    try {
        const action = body.queryResult.action; // Sử dụng action để match
        const parameters = body.queryResult.parameters;
        ...
        if (action === 'timVeXe') { // Khớp với action bạn set ở Dialogflow
            // Handle array cho diemDi/diemDen
